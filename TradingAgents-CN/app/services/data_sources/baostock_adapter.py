    ...
    def get_daily_basic(self, trade_date: str, max_stocks: int = None) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            try:
                ...
                # 预过滤：排除创业板/科创板/北交所/ST（与 stock_filter.py 一致）
                from .manager import should_exclude_stock
                filtered_stocks = []
                for s in stock_list:
                    if len(s) > 5 and s[4] == '1' and s[5] == '1':
                        code = s[0] if len(s) > 0 else ''
                        name = s[1] if len(s) > 1 else ''
                        if not should_exclude_stock(code, name):
                            filtered_stocks.append(s)

                total_before = len([s for s in stock_list if len(s) > 5 and s[4] == '1' and s[5] == '1'])
                total_stocks = len(filtered_stocks)
                logger.info(f"📊 BaoStock: 活跃股票 {total_before} 只，过滤后 {total_stocks} 只（排除创业板/科创板/北交所/ST），开始处理{'全部' if max_stocks is None else f'前 {max_stocks} 只'}...")
