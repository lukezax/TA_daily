...
# ==================== 全局股票过滤器 ====================
# 排除不需要同步的股票：创业板(300/301/302)、科创板(688)、北交所(8/4开头)、ST股票
# 与 stock_filter.py 的排除规则保持一致
EXCLUDED_CODE_PREFIXES = ('300', '301', '302', '688', '8', '4')


def should_exclude_stock(code: str, name: str = "") -> bool:
    """
    判断股票是否应被排除（不参与数据同步）
    ...
    Args:
        code: 股票代码（纯数字6位，或带后缀如 000001.SZ）
        name: 股票名称（用于判断 ST）

    Returns:
        True = 应排除, False = 保留
    """
    # 提取纯数字代码（支持格式：000001, 000001.SZ, sh.000001, sz.000001）
    code_clean = code.strip()
    if '.' in code_clean:
        parts = code_clean.split('.')
        # 找到数字部分
        code_only = next((p for p in parts if p.isdigit()), parts[0])
    else:
        code_only = code_clean

    # 代码前缀排除
    for prefix in EXCLUDED_CODE_PREFIXES:
        if code_only.startswith(prefix):
            return True
...
def filter_stock_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """
    过滤 DataFrame 中的股票，移除不需要的股票

    支持的列名：symbol, ts_code, code, 股票代码（自动识别）
    名称列：name, stock_name, 股票名称
    ...
    # 识别代码列
    code_col = None
    for col in ('symbol', 'ts_code', 'code', '股票代码'):
        if col in df.columns:
            code_col = col
            break

    if code_col is None:
        return df  # 无法识别代码列，不过滤

    # 识别名称列
    name_col = None
    for col in ('name', 'stock_name', '股票名称'):
        if col in df.columns:
            name_col = col
            break

    # 构建过滤 mask
    mask = df[code_col].apply(
        lambda c: not should_exclude_stock(
            str(c),
            str(df.loc[df[code_col] == c, name_col].iloc[0]) if name_col and not df.loc[df[code_col] == c].empty else ""
        )
    )
    ...
    for _, row in df.iterrows():
        ...
        keep_mask.append(not should_exclude_stock(code, name))

    filtered_df = df[keep_mask].reset_index(drop=True)
    removed = original_count - len(filtered_df)

    if removed > 0:
        logger.info(f"📋 [股票过滤] 已过滤 {removed} 只股票（创业板/科创板/北交所/ST），保留 {len(filtered_df)} 只")
        ...
        for adapter in available_adapters:
            try:
                ...
                if df is not None and not df.empty:
                    # 全局过滤：排除创业板/科创板/北交所/ST
                    df = filter_stock_dataframe(df)
