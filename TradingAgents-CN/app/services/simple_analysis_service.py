...
def get_provider_and_url_by_model_sync(model_name: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            # 🔥 尝试模型级别的环境变量覆盖（{MODEL_NAME}_BASE_URL / {MODEL_NAME}_API_KEY）
            model_env_key = model_name.upper().replace('-', '_').replace('.', '_')
            final_backend_url = os.environ.get(f"{model_env_key}_BASE_URL", backend_url)
            final_api_key = os.environ.get(f"{model_env_key}_API_KEY", api_key)
            if final_backend_url != backend_url:
                logger.info(f"✅ [同步查询] 模型 {model_name} 使用环境变量覆盖的 BASE_URL: {final_backend_url}")
            if final_api_key != api_key:
                logger.info(f"✅ [同步查询] 模型 {model_name} 使用环境变量覆盖的 API_KEY")
        ...
        # 🔥 尝试模型级别的环境变量覆盖（{MODEL_NAME}_BASE_URL / {MODEL_NAME}_API_KEY）
        model_env_key = model_name.upper().replace('-', '_').replace('.', '_')
        final_backend_url = os.environ.get(f"{model_env_key}_BASE_URL", _get_default_backend_url(provider_key))
        final_api_key = os.environ.get(f"{model_env_key}_API_KEY", _get_env_api_key_for_provider(provider_key))
        if final_backend_url != _get_default_backend_url(provider_key):
            logger.info(f"✅ [同步查询] 模型 {model_name} 使用环境变量覆盖的 BASE_URL: {final_backend_url}")
        if final_api_key != _get_env_api_key_for_provider(provider_key):
            logger.info(f"✅ [同步查询] 模型 {model_name} 使用环境变量覆盖的 API_KEY")
        return {
            "provider": provider_key,
            "backend_url": final_backend_url,
        ...
        # 🔥 尝试模型级别的环境变量覆盖（{MODEL_NAME}_BASE_URL / {MODEL_NAME}_API_KEY）
        model_env_key = model_name.upper().replace('-', '_').replace('.', '_')
        final_backend_url = os.environ.get(f"{model_env_key}_BASE_URL", _get_default_backend_url(provider))
        final_api_key = os.environ.get(f"{model_env_key}_API_KEY", _get_env_api_key_for_provider(provider))
        if final_backend_url != _get_default_backend_url(provider):
            logger.info(f"✅ [同步查询] 模型 {model_name} 使用环境变量覆盖的 BASE_URL: {final_backend_url}")
        if final_api_key != _get_env_api_key_for_provider(provider):
            logger.info(f"✅ [同步查询] 模型 {model_name} 使用环境变量覆盖的 API_KEY")
        return {
            "provider": provider,
            "backend_url": final_backend_url,
...
def create_analysis_config(
    ...
    try:
        ...
        # 🔥 保存深度模型的独立 backend_url 和 provider（用于混合模式）
        config["deep_backend_url"] = deep_provider_info.get("backend_url", config["backend_url"])
        config["deep_provider"] = deep_provider_info.get("provider", llm_provider)
        config["quick_backend_url"] = quick_provider_info.get("backend_url", config["backend_url"])
        config["quick_provider"] = quick_provider_info.get("provider", llm_provider)
        ...
        logger.info(f"🔧 深度模型 backend_url: {config['deep_backend_url']}")
        logger.info(f"🔧 快速模型 provider: {config['quick_provider']}, 深度模型 provider: {config['deep_provider']}")
    except Exception as e:
...
class SimpleAnalysisService:
    ...
    async def execute_analysis_background(
        ...
        try:
            ...
            # 数据验证加 5 分钟超时，防止外部 API 无响应导致事件循环挂死
            ...
            logger.error(f"❌ 分析任务超时: {task_id} - 数据准备阶段超过5分钟未响应")
            timeout_error = (
                "❌ 数据准备超时\n\n"
                "股票数据验证阶段超过5分钟未响应，可能是外部数据源不可用。\n\n"
                "💡 请稍后重试，或检查数据源服务状态"
            )
            if progress_tracker:
                progress_tracker.mark_failed(timeout_error)
            await self.memory_manager.update_task_status(
                task_id=task_id, status=TaskStatus.FAILED, progress=0,
                message="数据准备超时", current_step="failed",
                error_message=timeout_error
            )
            await self._update_task_status(task_id, AnalysisStatus.FAILED, 0, timeout_error)
    ...
    def _run_analysis_sync(
        ...
        try:
            ...
            # 🔷 服务端深度模式覆盖（python -m app --deep）
            try:
                from app.core.unified_config import unified_config
                _sys_settings = unified_config.get_system_settings()
                _deep_override = _sys_settings.get("deep_analysis_model", "")
                if _deep_override and _deep_override not in ("local", "qwen-turbo", "qwen-plus", "") and research_depth in ("深度", "全面"):
                    if _deep_override != deep_model:
                        logger.info(f"🔷 [深度模式] 服务端覆盖深度模型: {deep_model} → {_deep_override}")
                        deep_model = _deep_override
                        _revalidation = capability_service.validate_model_pair(
                            quick_model, deep_model, research_depth
                        )
                        if not _revalidation["valid"]:
                            for w in _revalidation["warnings"]:
                                logger.warning(w)
                            logger.warning(f"⚠️ [深度模式] 覆盖模型验证失败，回退到推荐模型")
                            quick_model, deep_model = capability_service.recommend_models_for_depth(
                                research_depth
                            )
                        else:
                            logger.info(f"✅ [深度模式] 覆盖模型验证通过: quick={quick_model}, deep={deep_model}")
            except Exception as _e:
                logger.debug(f"[深度模式] 检查服务端覆盖失败（可忽略）: {_e}")
            ...
            # ⚡ 轻量模式：从请求参数或服务端设置读取
            _lite_from_request = False
            if request.parameters and hasattr(request.parameters, 'lite_mode'):
                _lite_from_request = bool(request.parameters.lite_mode)
            _lite_from_settings = False
            try:
                from app.core.unified_config import unified_config
                _lite_from_settings = bool(unified_config.get_system_settings().get("lite_mode", False))
            except Exception:
                pass
            config["lite_mode"] = _lite_from_request or _lite_from_settings
            if config["lite_mode"]:
                logger.info("⚡ [轻量模式] 辩论节点将使用云端模型")
    ...
    async def _save_modular_reports_to_data_dir(self, result: Dict[str, Any], stock_symbol: str) -> Dict[str, str]:
        ...
        try:
            ...
            report_modules = {
                ...
                'czsc_report': {
                    'filename': 'czsc_report.md',
                    'title': f'{stock_symbol} 缠论结构分析报告',
                    'state_key': 'czsc_report'
                },
                'yangjia_report': {
                    'filename': 'yangjia_report.md',
                    'title': f'{stock_symbol} 养家视角分析报告',
                    'state_key': 'yangjia_report'
                },
                'investment_plan': {
