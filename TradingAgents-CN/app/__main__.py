...
def main():
    """主启动函数"""
    import argparse
    parser = argparse.ArgumentParser(description="TradingAgents-CN Backend Server")
    parser.add_argument("--deep", action="store_true", help="深度模式：深度分析使用 qwen3.7-max 云端模型（token-plan）")
    parser.add_argument("--lite", action="store_true", help="轻量模式：辩论节点使用云端模型，数据收集保持本地")
    ...
    if args.deep:
        try:
            ...
            settings_cache = unified_config.get_system_settings()
            settings_cache["deep_analysis_model"] = "qwen3.7-max"
            logger.info("🔷 深度模式已启用：深度分析模型 → qwen3.7-max（token-plan 云端）")
        except Exception as e:
            logger.warning(f"⚠️ 无法设置深度模式配置: {e}")

    if args.lite:
        try:
            from app.core.unified_config import unified_config
            settings_cache = unified_config.get_system_settings()
            settings_cache["lite_mode"] = True
            settings_cache["deep_analysis_model"] = "qwen3.7-max"
            logger.info("⚡ 轻量模式已启用：辩论节点 → qwen3.7-max（token-plan 云端），数据收集保持本地")
        except Exception as e:
            logger.warning(f"⚠️ 无法设置轻量模式配置: {e}")
