...
class TushareProvider(BaseStockDataProvider):
    ...
    async def get_historical_data(
        ...
        try:
            ...
            # 🔧 使用 pro.daily 接口获取非复权数据
            # 注意：120积分token只有 pro.daily() 权限（50次/分钟，8000次/天）
            # pro_bar() 需要 adj_factor 接口权限（1次/分钟），不可用
            # 非复权数据对于短期波段分析（1-3周）影响极小
