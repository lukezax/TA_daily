...
class DataSourceManager:
    ...
    def __init__(self):
        ...
        # 智兔 API 股票名称映射缓存（启动时加载一次）
        self._zhitu_stock_name_map: Dict[str, str] = {}
        try:
            self._load_zhitu_stock_name_map()
        except Exception as e:
            logger.warning(f"⚠️ 智兔股票名称映射加载失败（不影响启动）: {e}")
        ...
        logger.info(f"   智兔名称映射: {'✅ 已加载 ' + str(len(self._zhitu_stock_name_map)) + ' 条' if self._zhitu_stock_name_map else '❌ 未加载'}")
    ...
    def _load_zhitu_stock_name_map(self):
        """
        从智兔 API 加载权威股票名称映射（code -> name）。
        启动时调用一次，后续 get_stock_info 直接查内存缓存。
        """
        try:
            import sys
            import os
            # 尝试导入 ZhituAdapter（路径可能不在 sys.path 中）
            ...
            if not adapter.is_available():
                logger.warning("⚠️ 智兔 API 不可用（ZHITU_API_TOKEN 未配置），跳过名称映射加载")
                return

            df = adapter.get_stock_list()
            if df is not None and not df.empty:
                self._zhitu_stock_name_map = dict(zip(df['symbol'], df['name']))
                logger.info(f"✅ 智兔股票名称映射加载成功: {len(self._zhitu_stock_name_map)} 条记录")
            else:
                logger.warning("⚠️ 智兔 API get_stock_list() 返回空数据，名称映射未加载")
        except ImportError as e:
            logger.warning(f"⚠️ 无法导入 ZhituAdapter，跳过名称映射加载: {e}")
        except Exception as e:
            logger.warning(f"⚠️ 加载智兔股票名称映射时出错: {e}")
    ...
    def _get_data_source_priority_order(self, symbol: Optional[str] = None) -> List[ChinaDataSource]:
        ...
        try:
            ...
            if config_data and config_data.get('data_source_configs'):
                ...
                source_mapping = {
                    ...
                    DataSourceCode.ZHITU: ChinaDataSource.ZHITU,
                }
        ...
        # 🔥 回退到默认顺序（兼容性）
        # 默认顺序：AKShare > BaoStock > Tushare > Zhitu(兜底)
        default_order = [
            ChinaDataSource.AKSHARE,
            ChinaDataSource.BAOSTOCK,
            ChinaDataSource.TUSHARE,
            ChinaDataSource.ZHITU,
        ]
        # 只返回可用的数据源
    ...
    def _check_available_sources(self) -> List[ChinaDataSource]:
        ...
        try:
            ...
            if config_data and config_data.get('data_source_configs'):
                ...
                enabled_sources_in_db = {'mongodb', 'tushare', 'akshare', 'baostock', 'zhitu'}
        except Exception as e:
        ...
        # 检查智兔 API
        if 'zhitu' in enabled_sources_in_db:
            zhitu_token = datasource_configs.get('zhitu', {}).get('api_key') or os.getenv('ZHITU_API_TOKEN')
            if zhitu_token and zhitu_token.strip():
                available.append(ChinaDataSource.ZHITU)
                source = "数据库配置" if datasource_configs.get('zhitu', {}).get('api_key') else "环境变量"
                logger.info(f"✅ 智兔 API 数据源可用且已启用 (Token来源: {source})")
            else:
                logger.warning("⚠️ 智兔 API 数据源不可用: ZHITU_API_TOKEN 未配置")
        else:
            logger.info("ℹ️ 智兔 API 数据源已在数据库中禁用")
    ...
    def get_stock_data(self, symbol: str, start_date: str = None, end_date: str = None, period: str = "daily") -> str:
        ...
        try:
            ...
            elif self.current_source == ChinaDataSource.ZHITU:
                result = self._get_zhitu_data(symbol, start_date, end_date, period)
                actual_source = "zhitu"
            elif self.current_source == ChinaDataSource.TUSHARE:
    ...
    def _get_zhitu_data(self, symbol: str, start_date: str, end_date: str, period: str = "daily") -> str:
        """使用智兔API获取股票数据"""
        logger.info(f"📊 [智兔] 调用参数: symbol={symbol}, start_date={start_date}, end_date={end_date}, period={period}")
        ...
        try:
            ...
            if not adapter.is_available():
                return f"❌ 智兔API不可用（无可用token）"

            # 周期映射
            period_map = {"daily": "day", "weekly": "week", "monthly": "month"}
            zhitu_period = period_map.get(period, "day")
            ...
            if zhitu_period == "day":
                limit = max(days_diff + 30, 300)
            elif zhitu_period == "week":
                limit = max(days_diff // 7 + 10, 60)
            else:
                limit = max(days_diff // 30 + 5, 24)

            # 获取K线数据
            kline_data = adapter.get_kline(symbol, period=zhitu_period, limit=limit)

            if not kline_data or len(kline_data) == 0:
                return f"❌ 智兔API未获取到{symbol}的{period}数据"
            ...
            if 'date' in df.columns:
                df['date'] = df['date'].astype(str).str[:10]  # 取前10字符 YYYY-MM-DD
            ...
            if df.empty:
                return f"❌ 智兔API获取的{symbol}数据在指定日期范围内为空"
            ...
            # 获取股票名称
            stock_name = self._zhitu_stock_name_map.get(symbol, f'股票{symbol}')

            # 格式化返回（与其他数据源一致）
            result = self._format_stock_data_response(df, symbol, stock_name, start_date, end_date)

            duration = time.time() - start_time
            logger.info(f"✅ [智兔] 成功获取数据: {symbol}, {len(df)}条记录, 耗时{duration:.2f}秒")
            return result

        except Exception as e:
            duration = time.time() - start_time
            logger.error(f"❌ [智兔] 获取数据失败: {symbol}, 错误: {e}, 耗时{duration:.2f}秒")
            return f"❌ 智兔API获取{symbol}数据失败: {str(e)}"
        ...
        for source in fallback_order:
            if source != self.current_source and source in self.available_sources:
                try:
                    ...
                    # 直接调用具体的数据源方法，避免递归
                    if source == ChinaDataSource.ZHITU:
                        result = self._get_zhitu_data(symbol, start_date, end_date, period)
                    elif source == ChinaDataSource.TUSHARE:
                        result = self._get_tushare_data(symbol, start_date, end_date, period)
                    elif source == ChinaDataSource.AKSHARE:
    ...
    def get_stock_info(self, symbol: str) -> Dict:
        ...
        优先级：智兔名称映射 → MongoDB → Tushare → AKShare → BaoStock
        """
        logger.info(f"📊 [数据来源: {self.current_source.value}] 开始获取股票信息: {symbol}")

        # 最高优先级：智兔 API 名称映射（内存缓存，启动时已加载）
        if self._zhitu_stock_name_map and symbol in self._zhitu_stock_name_map:
            zhitu_name = self._zhitu_stock_name_map[symbol]
            logger.info(f"✅ [数据来源: 智兔名称映射] 命中缓存: {symbol} -> {zhitu_name}")
            return {
                'symbol': symbol,
                'name': zhitu_name,
                'area': '未知',
