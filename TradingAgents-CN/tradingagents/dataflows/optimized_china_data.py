...
"""
        ...
        try:
            ...
            if use_app_cache_enabled(False):
                ...
                if financial_data:
                    ...
                    logger.info(f"🔄 MongoDB 未找到{symbol}财务数据，尝试从智兔 API 获取")
            else:
                logger.info(f"🔄 数据库缓存未启用，尝试从智兔 API 获取{symbol}财务数据")

            # 第二优先级：从智兔 API 获取财务数据
            try:
                import sys
                ...
                if zhitu.is_available():
                    logger.info(f"🔍 [智兔API] 尝试获取{symbol}财务数据")
                    zhitu_data = zhitu.get_comprehensive_financial_data(symbol)
                    if zhitu_data:
                        logger.info(f"✅ [智兔API] 财务数据获取成功: {symbol}")
                        metrics = self._parse_zhitu_financial_data(zhitu_data, price_value)
                        if metrics:
                            logger.info(f"✅ [智兔API] 财务数据解析成功，返回指标")
                            return metrics
                        else:
                            logger.warning(f"⚠️ [智兔API] 财务数据解析失败，降级到 AKShare")
                    else:
                        logger.warning(f"⚠️ [智兔API] 未获取到{symbol}财务数据，降级到 AKShare")
                else:
                    logger.info(f"🔄 [智兔API] 未配置token，跳过，尝试 AKShare")
            except Exception as e:
                logger.warning(f"⚠️ [智兔API] 获取{symbol}财务数据异常: {e}，降级到 AKShare")
    ...
    def _parse_zhitu_financial_data(self, zhitu_data: dict, price_value: float) -> dict:
        """解析智兔 API 综合财务数据为标准指标格式"""
        try:
            logger.debug(f"📊 [智兔数据解析] 开始解析, 数据字段: {list(zhitu_data.keys())}")

            metrics = {}

            # ROE
            roe = zhitu_data.get("roe")
            if roe is not None and -200 <= roe <= 200:
                metrics["roe"] = f"{roe:.1f}%"
            else:
                metrics["roe"] = "N/A"

            # ROA（智兔可能不直接提供，设为 N/A）
            metrics["roa"] = "N/A"

            # 毛利率
            gross_margin = zhitu_data.get("gross_margin")
            if gross_margin is not None and -100 <= gross_margin <= 100:
                metrics["gross_margin"] = f"{gross_margin:.1f}%"
            else:
                metrics["gross_margin"] = "N/A"

            # 净利率
            net_margin = zhitu_data.get("net_margin")
            if net_margin is not None and -100 <= net_margin <= 100:
                metrics["net_margin"] = f"{net_margin:.1f}%"
            else:
                metrics["net_margin"] = "N/A"

            # PE
            pe = zhitu_data.get("pe")
            if pe is not None and pe > 0:
                metrics["pe"] = f"{pe:.1f}倍"
                metrics["pe_ttm"] = f"{pe:.1f}倍"  # 智兔的 PE 通常是 TTM
            else:
                metrics["pe"] = "N/A"
                metrics["pe_ttm"] = "N/A"

            # PB
            pb = zhitu_data.get("pb")
            if pb is not None and pb > 0:
                metrics["pb"] = f"{pb:.2f}倍"
            else:
                metrics["pb"] = "N/A"

            # 总市值 - 智兔 sz 字段单位为"百元"，即 实际市值(元) = sz * 100
            total_mv = zhitu_data.get("total_mv")
            if total_mv is not None and total_mv > 0:
                total_mv_yi = total_mv * 100 / 1e8  # 百元 → 元 → 亿元
                metrics["total_mv"] = f"{total_mv_yi:.2f}亿元"
            else:
                metrics["total_mv"] = "N/A"
            ...
            # 根据增长率调整
            revenue_growth = zhitu_data.get("revenue_growth")
            profit_growth = zhitu_data.get("profit_growth")
            ...
            metrics["fundamental_score"] = min(10, max(1, fundamental_score))
            metrics["valuation_score"] = min(10, max(1, valuation_score))
            metrics["growth_score"] = min(10, max(1, growth_score))

            # 风险等级
            avg_score = (metrics["fundamental_score"] + metrics["valuation_score"] + metrics["growth_score"]) / 3
            if avg_score >= 7:
                metrics["risk_level"] = "低风险"
            elif avg_score >= 5:
                metrics["risk_level"] = "中等风险"
            else:
                metrics["risk_level"] = "高风险"

            metrics["data_source"] = "智兔API"

            logger.info(f"✅ [智兔数据解析] 成功: PE={metrics.get('pe')}, PB={metrics.get('pb')}, ROE={metrics.get('roe')}")
            return metrics

        except Exception as e:
            logger.error(f"❌ [智兔数据解析] 失败: {e}", exc_info=True)
            return None
