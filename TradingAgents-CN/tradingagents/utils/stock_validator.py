        ...
        try:
            ...
            for data_source in priority_order:
                try:
                    ...
                    # 根据数据源获取对应的同步服务
                    if data_source == "zhitu":
                        # 智兔API直接通过adapter获取数据并写入数据库
                        from app.services.data_sources.zhitu_adapter import ZhituAdapter
                        zhitu = ZhituAdapter()
                        if not zhitu.is_available():
                            logger.warning(f"⚠️ [数据同步] 智兔API不可用，跳过")
                            last_error = f"{data_source}: API不可用"
                            continue

                        kline_data = zhitu.get_kline(stock_code, period="day", limit=400)
                        if kline_data and len(kline_data) > 0:
                            ...
                            if records:
                                ...
                                logger.info(f"✅ [数据同步] 智兔数据写入MongoDB: {historical_records}条")

                                return {
                                    "success": True,
                                    "message": f"使用zhitu同步成功: 历史{historical_records}条",
                                ...
                                last_error = f"{data_source}: 获取数据为空"
                                continue
                        else:
                            last_error = f"{data_source}: 获取K线失败"
                            continue
                    ...
                    # 2.3 同步实时行情（非交易时间跳过，避免外部API无响应导致挂死）
                    ...
                    def _is_trading_hours() -> bool:
                        ...
                        if now.weekday() >= 5:
                            return False
                        t = now.time()
                        morning = _time(9, 15) <= t <= _time(11, 35)
                        afternoon = _time(12, 55) <= t <= _time(15, 5)
                    ...
                    if _is_trading_hours():
                        ...
                        try:
                            ...
                            # 加 30 秒超时保护，防止外部 API 无响应导致事件循环挂死
                            import asyncio as _asyncio
                            rt_result = await _asyncio.wait_for(
                                realtime_service.sync_realtime_quotes(
                                    symbols=[stock_code],
                                    force=True
                                ),
                                timeout=30
                            ...
                            logger.warning(f"⚠️ [数据同步] 实时行情同步超时(30s)，跳过")
                        except Exception as e:
                            logger.warning(f"⚠️ [数据同步] 实时行情同步异常: {e}")
                    else:
                        logger.info(f"⏰ [数据同步] 非交易时间，跳过实时行情同步")
    ...
    def _get_data_source_priority_for_sync(self, stock_code: str) -> list:
        ...
        try:
            ...
            if adapter.use_app_cache and adapter.db is not None:
                ...
                return ['akshare', 'baostock', 'tushare', 'zhitu']
