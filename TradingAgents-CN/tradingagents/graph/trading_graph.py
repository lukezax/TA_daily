...
def create_llm_by_provider(provider: str, model: str, backend_url: str, temperature: float, max_tokens: int, timeout: int, api_key: str = None, **extra_kwargs):
    ...
    if normalized_provider == "google":
        ...
        from tradingagents.llm_clients.openai_client import (
            _is_local_url, _clear_proxy_env, _restore_proxy_env, _proxy_env_lock
        )

        need_bypass = _is_local_url(backend_url)
        if need_bypass:
            logger.info(f"🔧 [代理绕过-fallback] 本地地址 {backend_url}，临时清除代理环境变量")
        ...
        if need_bypass:
            with _proxy_env_lock:
                ...
                try:
                    result_llm = ChatOpenAI(**llm_kwargs)
                except Exception as e:
                    logger.error(f"❌ [fallback LLM创建失败] {e}", exc_info=True)
                    raise
                finally:
...
class TradingAgentsGraph:
    ...
    def __init__(
        ...
        # 读取快速模型参数
        quick_max_tokens = quick_config.get("max_tokens", 131072)
        quick_temperature = quick_config.get("temperature", 0.7)
        quick_timeout = quick_config.get("timeout", 600)

        # 读取深度模型参数
        deep_max_tokens = deep_config.get("max_tokens", 131072)
        ...
        if normalized_quick_provider and normalized_deep_provider and normalized_quick_provider != normalized_deep_provider:
            ...
            # 🔥 支持快速模型和深度模型使用不同的 backend_url（如 deep 走云上 token-plan）
            deep_backend_url = self.config.get("deep_backend_url") or self.config.get("backend_url")
            quick_backend_url = self.config.get("quick_backend_url") or self.config.get("backend_url")

            if deep_backend_url != quick_backend_url:
                logger.info(f"🔀 [Qwen 分离模式] 深度模型 backend_url: {deep_backend_url}")
                logger.info(f"🔀 [Qwen 分离模式] 快速模型 backend_url: {quick_backend_url}")
                self.deep_thinking_llm = create_llm_by_provider(
                    provider="qwen",
                    model=self.config["deep_think_llm"],
    ...
    def propagate(self, company_name, trade_date, progress_callback=None, task_id=None):
        ...
        if self.debug:
            ...
            for chunk in self.graph.stream(init_agent_state, **args):
                ...
                if progress_callback and args.get("stream_mode") == "updates":
                    ...
                    if chunk and len(chunk.get("messages", [])) > 0:
        ...
        # TDD 式报告校验：验证 investment_plan 和 trader_investment_plan 的数据一致性
        try:
            validator = ReportValidator(self.deep_thinking_llm)
            final_state = validator.validate_and_retry(final_state, "investment_plan")
            if not final_state.get("_validation_failed"):
                final_state = validator.validate_and_retry(final_state, "trader_investment_plan")
            if final_state.get("_validation_failed"):
                logger.error(f"❌ [报告校验] 校验失败: {final_state.get('_validation_failure_reason', '')}")
        except Exception as e:
            logger.error(f"❌ [报告校验] 校验过程异常: {e}")
