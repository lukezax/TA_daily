...
- 如果工具返回"无法获取"或"数据不可用"，你必须如实报告数据不可用，不得编造替代内容
- 你的分析必须100%基于工具返回的真实数据，不允许凭记忆补充任何具体的新闻事件或讨论内容
- 如果数据不足，直接说明"数据不足，无法进行完整的情绪分析"即可"""
        )
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    ...
                    "- 公司名称：{company_name}\n"
                    "- 股票代码：{ticker}\n"
                    "⚠️ 严禁使用其他公司名称。如果工具返回的数据中包含公司名称，以工具数据为准。\n"
                    "标的约束：{instrument_context}\n{system_message}"
                    "供您参考，当前日期是{current_date}。我们要分析的当前公司是{ticker}（{company_name}）。请用中文撰写所有分析内容。",
                ),
