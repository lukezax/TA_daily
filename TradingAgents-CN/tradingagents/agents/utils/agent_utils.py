    ...
    def get_china_market_overview(
        ...
        try:
            ...
            if zhitu.is_available():
                overview = zhitu.get_market_overview()
                if overview:
                    ...
                    for key, display_name in name_map.items():
                        ...
                        if idx:
                            ...
                            pct = idx.get('change_pct', 0) or 0  # 已经是百分比值
                            arrow = "↑" if change > 0 else ("↓" if change < 0 else "→")
                            report += f"### {display_name} ({idx.get('code', '')})\n"
                            report += f"- 最新点位: {price}\n"
                            report += f"- 涨跌: {change:+.2f} ({pct:+.2f}%) {arrow}\n"
                            report += f"- 今开: {idx.get('open', 'N/A')} | 最高: {idx.get('high', 'N/A')} | 最低: {idx.get('low', 'N/A')}\n"
                            report += f"- 成交额: {idx.get('amount', 'N/A')}\n\n"
                        else:
        ...
        try:
            ...
            if is_china or is_hk:
                ...
                try:
                    ...
                    zhitu = ZhituAdapter()
                    if zhitu.is_available():
                        profile = zhitu.get_company_profile(ticker)
                ...
                # === 主数据源：多源搜索（BochaAI → Exa → SerpAPI → Tavily 自动轮换）===
                try:
                    from tradingagents.utils.search_client import get_search_client
                    import concurrent.futures

                    search_client = get_search_client()
                    search_query = f"{ticker} {company_name} 最新消息 投资者情绪 股吧讨论"
                    logger.info(f"🔍 [统一情绪工具] 多源搜索: {company_name}({ticker})")

                    # 使用线程池+超时
                    def _multi_search():
                        return search_client.search(search_query, max_results=8)

                    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                        future = executor.submit(_multi_search)
                        try:
                            search_results = future.result(timeout=30)
                        except concurrent.futures.TimeoutError:
                            logger.warning(f"⚠️ [统一情绪工具] 搜索超时(30s)，降级到东方财富新闻")
                            search_results = None

                    if search_results and len(search_results) > 0:
                        actual_source = search_results[0].get("source", "unknown")
                        sources_text = "\n".join(
                            f"- [{r.get('title', '')[:50]}]({r.get('url', '')})"
                            for r in search_results[:5] if r.get("title")
                        )
                        content_text = "\n\n".join(
                            f"**{r.get('title', '')}**\n{r.get('content', '')}"
                        ...
                        sentiment_summary = f"""
## {company_name}（{ticker}）市场情绪与舆情分析

**分析日期**: {curr_date}
**数据来源**: 互联网实时搜索（via {actual_source}）
...
"""
                        ...
                        tavily_success = True
                        logger.info(f"✅ [统一情绪工具] 搜索成功 [{actual_source}]，{len(search_results)}条结果")
                        logger.info(f"📊 [搜索用量] {search_client.get_usage_summary()}")

                except ImportError as e:
                    logger.warning(f"⚠️ [统一情绪工具] 搜索模块导入失败: {e}，降级到东方财富新闻")
                except Exception as e:
                    logger.warning(f"⚠️ [统一情绪工具] 搜索失败: {e}，降级到东方财富新闻")

                # === 补充/Fallback：东方财富新闻情绪分析 ===
                try:
                    import akshare as ak
                    code_only = ticker.replace('.SH', '').replace('.SZ', '').zfill(6)
                    news_df = ak.stock_news_em(symbol=code_only)

                    if news_df is not None and not news_df.empty:
                        positive_words = ['上涨', '增长', '利好', '看好', '买入', '推荐', '强势', '突破',
                                         '创新高', '盈利', '超预期', '加仓', '龙头', '景气']
                        negative_words = ['下跌', '下降', '利空', '看空', '卖出', '风险', '跌破',
                                         '创新低', '亏损', '减持', '暴跌', '警示', '退市', '违规']
                        ...
                        for _, row in news_df.head(10).iterrows():
                            ...
                            news_items_text.append(f"- [{sentiment_label}] {title} ({source}, {pub_time})")
                        ...
                        news_summary = f"""
## {company_name}（{ticker}）新闻情绪量化 {sentiment_emoji}

**数据来源**: 东方财富实时新闻（{total}条）
- **整体情绪**: {overall_sentiment}（评分: {sentiment_score:+.2f}）
- **正面/负面/中性**: {positive_count}/{negative_count}/{neutral_count}

### 最新新闻
{news_list}
"""
                        result_data.append(news_summary)
                        logger.info(f"✅ [统一情绪工具] 东方财富新闻情绪: {overall_sentiment} ({total}条)")

                    elif not tavily_success:
                        # 两个数据源都失败
                        result_data.append(f"""
## {company_name}（{ticker}）市场情绪分析

**数据状态**: ⚠️ 无法获取情绪数据
当前无法获取 {company_name} 的新闻和社媒数据。请以技术面和基本面分析为主要决策依据。
""")

                except Exception as e:
                    if not tavily_success:
                        logger.error(f"❌ [统一情绪工具] 所有情绪数据源失败: {e}")
                        result_data.append(f"""
## {company_name}（{ticker}）市场情绪分析

**数据状态**: ❌ 获取失败
无法获取情绪数据，请以技术面和基本面分析为主要决策依据。
""")
