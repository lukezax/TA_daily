...
# 代理绕过：本地地址创建 OpenAI 客户端时临时清除 socks 代理环境变量
_PROXY_ENV_KEYS = ("ALL_PROXY", "all_proxy", "HTTP_PROXY", "http_proxy", "HTTPS_PROXY", "https_proxy")
_proxy_lock = threading.Lock()


def _create_openai_bypass_proxy(**kwargs) -> OpenAI:
    """创建 OpenAI 客户端，如果 base_url 是本地地址则临时清除代理环境变量"""
    ...
    def __init__(self, name, config):
        ...
        if self.llm_provider == "dashscope" or self.llm_provider == "alibaba":
            ...
            if not dashscope_key or force_openai:
                ...
                if openai_key:
                    self.client = _create_openai_bypass_proxy(
                        api_key=openai_key,
            ...
            if dashscope_key:
                try:
                    ...
                    if openai_key:
                        ...
                        self.fallback_client = _create_openai_bypass_proxy(api_key=openai_key, base_url=config["backend_url"])
            ...
            self.client = _create_openai_bypass_proxy(base_url=config["backend_url"])
        else:
            self.embedding = "text-embedding-3-small"
            openai_key = os.getenv('OPENAI_API_KEY')
            if openai_key:
