...
def create_bear_researcher(llm, memory):
    def bear_node(state) -> dict:
        ...
        correct_name_from_report = company_name  # fallback
        correct_price = "未知"
        if market_research_report:
            title_match = re.search(r'\*\*(.+?)（\d{6}）', market_research_report)
            if title_match:
                correct_name_from_report = title_match.group(1)

        # 使用统一的价格提取工具（增强正则 + 数据源 fallback）
        from tradingagents.utils.stock_utils import extract_price_with_fallback
        correct_price = extract_price_with_fallback(ticker, market_research_report)

        prompt = f"""📌 关键事实（来自实时行情工具验证，不可更改）：
- 公司名称：{correct_name_from_report}
- 股票代码：{ticker}
- 当前价格：¥{correct_price}
请确保你的分析严格基于以上事实，不要使用其他公司名称或价格。

你是一位看跌分析师，负责论证不投资股票 {correct_name_from_report}（股票代码：{ticker}）的理由。
...
⏰ 交易框架提醒：本系统服务于1-3周的波段交易。请聚焦短期内可能导致下跌的实质性风险（如业绩暴雷、大股东减持、行业政策利空、技术面破位），而非中长期估值偏高本身。PE在50-200范围内对短期走势影响有限，不要将其作为看跌的主要论据。
